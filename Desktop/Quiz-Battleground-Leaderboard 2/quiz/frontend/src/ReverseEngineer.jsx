...
function ReverseEngineering() {
  ...
  const clearQuizStorage = () => {
    [
      "reverse-engineering_score",
      "reverse-engineering_round",
      "reverse-engineering_currentIndex",
      "reverse-engineering_answers",
      "reverse-engineering_quizEnded",
      "reverse-engineering_questions",
      "reverse-engineering_allQuestions",
    ].forEach((key) => localStorage.removeItem(key));

    for (let i = 0; i < 100; i++) {
      localStorage.removeItem(`reverse-engineering_questionStart_${i}`);
    }
  };
  ...
  useEffect(() => {
    const initializeQuiz = async () => {
      try {
        const savedAllQuestions = localStorage.getItem("reverse-engineering_allQuestions");
        const savedQuestions = localStorage.getItem("reverse-engineering_questions");
        const savedScore = localStorage.getItem("reverse-engineering_score");
        const savedRound = localStorage.getItem("reverse-engineering_round");
        const savedCurrentIndex = localStorage.getItem("reverse-engineering_currentIndex");
        const savedAnswers = localStorage.getItem("reverse-engineering_answers");
        const savedQuizEnded = localStorage.getItem("reverse-engineering_quizEnded");
  ...
  useEffect(() => {
    if (!isLoading)
      localStorage.setItem("reverse-engineering_score", JSON.stringify(score));
  }, [score, isLoading]);
  useEffect(() => {
    if (!isLoading) localStorage.setItem("reverse-engineering_round", round.toString());
  }, [round, isLoading]);
  useEffect(() => {
    if (!isLoading)
      localStorage.setItem("reverse-engineering_currentIndex", currentIndex.toString())
  }, [currentIndex, isLoading]);
  useEffect(() => {
    if (!isLoading)
      localStorage.setItem("reverse-engineering_answers", JSON.stringify(answers));
  }, [answers, isLoading]);
  useEffect(() => {
    if (!isLoading)
      localStorage.setItem("reverse-engineering_quizEnded", quizEnded.toString());
  }, [quizEnded, isLoading]);
  useEffect(() => {
    if (!isLoading && questions.length)
      localStorage.setItem("reverse-engineering_questions", JSON.stringify(questions));
  }, [questions, isLoading]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (!localStorage.getItem(`reverse-engineering_questionStart_${currentIndex}`)) {
      localStorage.setItem(
        `reverse-engineering_questionStart_${currentIndex}`,
        Date.now().toString()
      );
    }
    ...
    const tick = () => {
      const startTime = Number(
        localStorage.getItem(`reverse-engineering_questionStart_${currentIndex}`)
      );
  ...
  const handleSubmit = () => {
    ...
    setTimeout(() => {
      ...
      localStorage.setItem(
        `reverse-engineering_questionStart_${currentIndex + 1}`,
        Date.now().toString()
      );
    }, 1500);

    localStorage.removeItem(`reverse-engineering_questionStart_${currentIndex}`);
  };
  ...
  const submitFinalResult = () => {
    ...
    axios
      .post("/api/user/playing-quiz", {
        email: user.email,
        nameCategory: "Reverse Engineering",
        round,
        questions: answers,
      })
      .then(() => {
        setQuizEnded(true);
        localStorage.setItem("reverse-engineering_quizEnded", "true");
        sessionStorage.setItem("quiz_active", "false");
      })
      .catch(console.error);
  };
