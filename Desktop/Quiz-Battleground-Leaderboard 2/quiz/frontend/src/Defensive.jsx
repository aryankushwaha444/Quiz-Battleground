...
function Defensive() {
  ...
  const clearQuizStorage = () => {
    [
      "defensive_score",
      "defensive_round",
      "defensive_currentIndex",
      "defensive_answers",
      "defensive_quizEnded",
      "defensive_questions",
      "defensive_allQuestions",
    ].forEach((key) => localStorage.removeItem(key));

    for (let i = 0; i < 100; i++) {
      localStorage.removeItem(`defensive_questionStart_${i}`);
    }
  };
  ...
  useEffect(() => {
    const initializeQuiz = async () => {
      try {
        const savedAllQuestions = localStorage.getItem("defensive_allQuestions");
        const savedQuestions = localStorage.getItem("defensive_questions");
        const savedScore = localStorage.getItem("defensive_score");
        const savedRound = localStorage.getItem("defensive_round");
        const savedCurrentIndex = localStorage.getItem("defensive_currentIndex");
        const savedAnswers = localStorage.getItem("defensive_answers");
        const savedQuizEnded = localStorage.getItem("defensive_quizEnded");
  ...
  useEffect(() => {
    if (!isLoading)
      localStorage.setItem("defensive_score", JSON.stringify(score));
  }, [score, isLoading]);
  useEffect(() => {
    if (!isLoading) localStorage.setItem("defensive_round", round.toString());
  }, [round, isLoading]);
  useEffect(() => {
    if (!isLoading)
      localStorage.setItem("defensive_currentIndex", currentIndex.toString())
  }, [currentIndex, isLoading]);
  useEffect(() => {
    if (!isLoading)
      localStorage.setItem("defensive_answers", JSON.stringify(answers));
  }, [answers, isLoading]);
  useEffect(() => {
    if (!isLoading)
      localStorage.setItem("defensive_quizEnded", quizEnded.toString());
  }, [quizEnded, isLoading]);
  useEffect(() => {
    if (!isLoading && questions.length)
      localStorage.setItem("defensive_questions", JSON.stringify(questions));
  }, [questions, isLoading]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (!localStorage.getItem(`defensive_questionStart_${currentIndex}`)) {
      localStorage.setItem(
        `defensive_questionStart_${currentIndex}`,
        Date.now().toString()
      );
    }
    ...
    const tick = () => {
      const startTime = Number(
        localStorage.getItem(`defensive_questionStart_${currentIndex}`)
      );
  ...
  const handleSubmit = () => {
    ...
    setTimeout(() => {
      ...
      localStorage.setItem(
        `defensive_questionStart_${currentIndex + 1}`,
        Date.now().toString()
      );
    }, 1500);

    localStorage.removeItem(`defensive_questionStart_${currentIndex}`);
  };
  ...
  const submitFinalResult = () => {
    ...
    axios
      ...
      .then(() => {
        setQuizEnded(true);
        localStorage.setItem("defensive_quizEnded", "true");
        sessionStorage.setItem("quiz_active", "false");
      })
      .catch(console.error);
  };
